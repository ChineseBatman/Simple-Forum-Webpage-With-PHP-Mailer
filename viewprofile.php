...
$check_user_info = mysqli_query($connect, "SELECT * FROM users WHERE user_id = '".$_GET['id']."'");
$rows = mysqli_num_rows($check_user_info);
while($row = mysqli_fetch_assoc($check_user_info)){
    $view_user_username = $row['user_username'];
    $view_user_email = $row['user_email'];
    $view_user_level = $row['user_level'];
    $view_user_profile_picture = $row['user_profile_picture'];
    $view_user_date_joined = $row['user_date'];
}
...
<head>
    ...
    <title><?php echo $view_user_username; ?> Profile</title>
...
<body>
    ...
    <div class="text-center profile-card" style="background-color:#ffffff;">
        ...
        <div><img class="rounded-circle" style="margin-top:-70px;" src="<?php echo $view_user_profile_picture; ?>" height="150px">
            <h3><?php echo $view_user_username; ?></h3>
            <p style="padding:10px;padding-bottom:0;padding-top:5px;">Email:&nbsp;<?php echo $view_user_email; ?></p>
            <p style="padding:10px;padding-bottom:0;padding-top:5px;">User Level:&nbsp;<?php echo $view_user_level; ?></p>
            <p style="padding:10px;padding-bottom:0;padding-top:5px;">Date Joined:&nbsp;<?php echo $view_user_date_joined; ?></p>
        </div>
    </div>
    </section>
    <section>
